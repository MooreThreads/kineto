...
class MockMuptiMetrics : public MuptiMetricApi {
 public:
  MockMuptiMetrics() : MuptiMetricApi(0) {}
  MOCK_METHOD1(idFromName, MUpti_MetricID(const std::string& name));
  MOCK_METHOD1(
      events,
      std::map<MUpti_EventID, std::string>(MUpti_MetricID metric_id));
  MOCK_METHOD1(valueKind, MUpti_MetricValueKind(MUpti_MetricID metric));
  MOCK_METHOD1(
      evaluationMode,
      MUpti_MetricEvaluationMode(MUpti_MetricID metric));
  MOCK_METHOD5(
      calculate,
      SampleValue(
...
TEST(MetricTest, Calculate) {
  ...
  EXPECT_CALL(metrics, valueKind(1))
      .Times(1)
      .WillOnce(Return(MUPTI_METRIC_VALUE_KIND_DOUBLE));
  Metric m(
      "ipc", 1, {2, 3}, MUPTI_METRIC_EVALUATION_MODE_PER_INSTANCE, metrics);

  // Calculate metric for first sample
  // Since evaluation mode is MUPTI_METRIC_EVALUATION_MODE_PER_INSTANCE,
  // Mupti API will be called three times: once for each SM (2) and once
  // to get the total across SMs.
  std::vector<MUpti_EventID> ids = {2, 3};
  std::vector<int64_t> vals = {100, 1000};
  EXPECT_CALL(
      metrics, calculate(1, MUPTI_METRIC_VALUE_KIND_DOUBLE, ids, vals, 1000))
      .Times(1)
  ...
  // Change evaluation mode to MUPTI_METRIC_EVALUATION_MODE_AGGREGATE.
  // Now we should get only one call to the Mupti API for the total.
  EXPECT_CALL(metrics, valueKind(1))
      .Times(1)
      .WillOnce(Return(MUPTI_METRIC_VALUE_KIND_DOUBLE));
  Metric m2("ipc", 1, {2, 3}, MUPTI_METRIC_EVALUATION_MODE_AGGREGATE, metrics);
  vals = {700, 2600};
  EXPECT_CALL(
      metrics, calculate(1, MUPTI_METRIC_VALUE_KIND_DOUBLE, ids, vals, 1000))
      .Times(1)
...
class MockMuptiEvents : public MuptiEventApi {
 public:
  MOCK_METHOD1(
      createGroupSets,
      MUpti_EventGroupSets*(std::vector<MUpti_EventID>& ids));
  MOCK_METHOD1(destroyGroupSets, void(MUpti_EventGroupSets* sets));
  MOCK_METHOD0(setContinuousMode, bool());
  MOCK_METHOD1(enablePerInstance, void(MUpti_EventGroup eventGroup));
  MOCK_METHOD1(instanceCount, uint32_t(MUpti_EventGroup eventGroup));
  MOCK_METHOD1(enableGroupSet, void(MUpti_EventGroupSet& set));
  MOCK_METHOD1(disableGroupSet, void(MUpti_EventGroupSet& set));
  MOCK_METHOD3(
      readEvent,
      void(MUpti_EventGroup g, MUpti_EventID id, std::vector<int64_t>& vals));
  MOCK_METHOD1(eventsInGroup, std::vector<MUpti_EventID>(MUpti_EventGroup g));
  MOCK_METHOD1(eventId, MUpti_EventID(const std::string& name));
};

TEST(EventGroupSetTest, CollectSample) {
  using ::testing::_;
  using ::testing::Return;
  const MUpti_EventGroup g1{nullptr};
  const MUpti_EventGroup g2{reinterpret_cast<void*>(0x1000)};
  MUpti_EventGroup groups[] = {g1, g2};
  ...
  MockMuptiEvents mupti_events;
  EXPECT_CALL(mupti_events, enablePerInstance(g1)).Times(1);
  EXPECT_CALL(mupti_events, enablePerInstance(g2)).Times(1);
  EXPECT_CALL(mupti_events, instanceCount(g1)).Times(1).WillOnce(Return(80));
  EXPECT_CALL(mupti_events, instanceCount(g2)).Times(1).WillOnce(Return(40));
  std::vector<MUpti_EventID> events_in_group1 = {4, 5};
  EXPECT_CALL(mupti_events, eventsInGroup(g1))
      .Times(1)
      .WillOnce(Return(events_in_group1));
  std::vector<MUpti_EventID> events_in_group2 = {10};
  EXPECT_CALL(mupti_events, eventsInGroup(g2))
      .Times(1)
      .WillOnce(Return(events_in_group2));
  EventGroupSet group_set(set, events, mupti_events);
  ...
  // This should not cause any Mupti API action as the group
  // set is already disabled
  group_set.setEnabled(false);

  // Activate group set - if activated twice, only the first
  // should cause mupti API to be called
  EXPECT_CALL(mupti_events, enableGroupSet(_)).Times(1);
  group_set.setEnabled(false);
  ...
  EXPECT_CALL(mupti_events, eventsInGroup(g2))
      .Times(1)
      .WillOnce(Return(events_in_group2));
  EXPECT_CALL(mupti_events, readEvent(g1, 4, _)).Times(1);
  EXPECT_CALL(mupti_events, readEvent(g1, 5, _)).Times(1);
  EXPECT_CALL(mupti_events, readEvent(g2, 10, _)).Times(1);
...
class EventProfilerTest : public ::testing::Test {
 protected:
  void SetUp() override {
    auto mupti_events_ptr = std::make_unique<MockMuptiEvents>();
    auto mupti_metrics_ptr = std::make_unique<MockMuptiMetrics>();
    muptiEvents_ = mupti_events_ptr.get();
    muptiMetrics_ = mupti_metrics_ptr.get();
    ...
    profiler_ = std::make_unique<EventProfiler>(
        std::move(mupti_events_ptr),
        std::move(mupti_metrics_ptr),
        loggers_,
        onDemandLoggers_);
  ...
  MockMuptiEvents* muptiEvents_;
  MockMuptiMetrics* muptiMetrics_;
  ...
  MUpti_EventGroup eventGroups_[kEventGroupCount];
  MUpti_EventGroupSet groupSet_[kGroupSetCount];
  MUpti_EventGroupSets groupSets_;
...
TEST_F(EventProfilerTest, ConfigureFailure) {
  ...
  EXPECT_CALL(*muptiEvents_, eventId("cycles"))
      .Times(2)
      .WillRepeatedly(Return(0));
  std::vector<MUpti_EventID> ids = {0};
  EXPECT_CALL(*muptiEvents_, createGroupSets(ids))
      .Times(2)
...
TEST_F(EventProfilerTest, ConfigureBase) {
  ...
  EXPECT_CALL(*muptiEvents_, eventId("elapsed_cycles_sm"))
      .Times(1)
      .WillOnce(Return(5));
  std::vector<MUpti_EventID> ids = {5};
  EXPECT_CALL(*muptiEvents_, createGroupSets(ids))
      .Times(1)
      .WillOnce(Return(&groupSets_));
  EXPECT_CALL(*muptiEvents_, enablePerInstance(eventGroups_[0])).Times(1);
  EXPECT_CALL(*muptiEvents_, instanceCount(eventGroups_[0]))
      .Times(1)
      .WillOnce(Return(80));
  EXPECT_CALL(*muptiEvents_, eventsInGroup(eventGroups_[0]))
      .Times(1)
      .WillOnce(Return(ids));
  EXPECT_CALL(*muptiEvents_, enableGroupSet(_)).Times(1);
...
TEST_F(EventProfilerTest, ConfigureOnDemand) {
  ...
  // One event
  EXPECT_CALL(*muptiEvents_, eventId("active_cycles"))
      .Times(1)
      .WillOnce(Return(3));
  // One metric
  EXPECT_CALL(*muptiMetrics_, idFromName("ipc")).Times(1).WillOnce(Return(10));
  std::map<MUpti_EventID, std::string> ipc_events;
  ipc_events[4] = "instructions";
  ipc_events[5] = "elapsed_cycles_sm";
  EXPECT_CALL(*muptiMetrics_, events(10)).Times(1).WillOnce(Return(ipc_events));
  EXPECT_CALL(*muptiMetrics_, evaluationMode(10))
      .Times(1)
      .WillOnce(Return(MUPTI_METRIC_EVALUATION_MODE_PER_INSTANCE));
  EXPECT_CALL(*muptiMetrics_, valueKind(10))
      .Times(1)
      .WillOnce(Return(MUPTI_METRIC_VALUE_KIND_DOUBLE));
  std::vector<MUpti_EventID> ids = {3, 4, 5};
  groupSet_[0].numEventGroups = 2;
  groupSets_.numSets = 2;
  EXPECT_CALL(*muptiEvents_, createGroupSets(ids))
      .Times(1)
      .WillOnce(Return(&groupSets_));
  // Specified MUPTI_METRIC_EVALUATION_MODE_PER_INSTANCE per instance above
  // So check that it's enabled
  EXPECT_CALL(*muptiEvents_, enablePerInstance(eventGroups_[0])).Times(1);
  EXPECT_CALL(*muptiEvents_, enablePerInstance(eventGroups_[1])).Times(1);
  EXPECT_CALL(*muptiEvents_, enablePerInstance(eventGroups_[2])).Times(1);
  std::vector<MUpti_EventID> ids_g1{3}, ids_g2{4}, ids_g3{5};
  EXPECT_CALL(*muptiEvents_, eventsInGroup(eventGroups_[0]))
      .Times(1)
      .WillOnce(Return(ids_g1));
  EXPECT_CALL(*muptiEvents_, eventsInGroup(eventGroups_[1]))
      .Times(1)
      .WillOnce(Return(ids_g2));
  EXPECT_CALL(*muptiEvents_, eventsInGroup(eventGroups_[2]))
      .Times(1)
...
TEST_F(EventProfilerTest, ReportSample) {
  ...
  EXPECT_CALL(*muptiEvents_, createGroupSets(ids))
      .Times(1)
      .WillOnce(Return(&groupSets_));
  EXPECT_CALL(*muptiEvents_, instanceCount(_))
      .Times(3)
  ...
  EXPECT_CALL(*muptiEvents_, readEvent(_, _, _))
      .Times(6)
      .WillRepeatedly(Invoke(
          [](MUpti_EventGroup g, MUpti_EventID id, std::vector<int64_t>& vals) {
            vals = {1, 2, 3, 4};
          }));
  ...
  profiler_->collectSample();
  EXPECT_CALL(*muptiEvents_, disableGroupSet(_)).Times(1);
  EXPECT_CALL(*muptiEvents_, enableGroupSet(_)).Times(1);
  profiler_->enableNextCounterSet();
  profiler_->collectSample();
  profiler_->collectSample();

  std::vector<MUpti_EventID> ipc_ids = {4, 5};
  // Called once for each instance (4) and once for the total.
  // x2 since we recompute per logger.
  EXPECT_CALL(
      *muptiMetrics_,
      calculate(10, MUPTI_METRIC_VALUE_KIND_DOUBLE, ipc_ids, _, 2000000000))
      .Times(10)
