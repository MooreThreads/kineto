...
TEST(ParseTest, ActivityTypes) {
  ...
  EXPECT_EQ(cfg.selectedActivityTypes(),
    std::set<ActivityType>({ActivityType::CPU_OP,
                            ...
                            ActivityType::MUSA_RUNTIME,
                            ActivityType::MUSA_DRIVER}));
  ...
  EXPECT_TRUE(cfg2.parse("ACTIVITY_TYPES = musa_Runtime,"));
  EXPECT_EQ(cfg2.selectedActivityTypes(),
    std::set<ActivityType>({ActivityType::MUSA_RUNTIME}));

  // Should throw an exception because incorrect activity name
  EXPECT_FALSE(cfg2.parse("ACTIVITY_TYPES = memcopy,musa_runtime"));
