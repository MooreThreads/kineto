...
#define DRIVER_API_CALL(apiFuncCall)                           \
  do {                                                         \
    ...
    if (_status != MUSA_SUCCESS) {                             \
      LOG(ERROR) << "Failed invoking MUSA driver function "    \
                 << #apiFuncCall << " status = "               \
                 << _status;                                   \
      exit(-1);                                                \
    }                                                          \
...
void VectorAddSubtract() {
  ...
  musaMemcpy(h_D, d_D, size, musaMemcpyDeviceToHost);
...
bool runTestWithAutoRange(
  ...
  // create a MUPTI range based profiling profiler
  ...
  MuptiRangeProfilerOptions opts{
    ...
    .maxRanges = 2,
    .numNestingLevels = 1,
    .muContext = async ? nullptr : MUcontext
  };
  MuptiRBProfilerSession profiler(opts);
...
bool runTestWithUserRange(
  ...
  MUpti_ProfilerRange profilerRange = MUPTI_UserRange;
  MUpti_ProfilerReplayMode profilerReplayMode = MUPTI_UserReplay;
...
int main(int argc, char* argv[]) {
  ...
  if (deviceCount == 0) {
    LOG(ERROR) << "There is no device supporting MUSA.";
    return -2;
  }
  ...
  if (argc > 1)
    ...
    deviceNum = 0;
  LOG(INFO) << "MUSA Device Number: " << deviceNum;
  ...
  if (computeCapabilityMajor < 7) {
    LOG(ERROR) << "MUPTI Profiler is not supported  with compute capability < 7.0";
    return -2;
  }

  MuptiRBProfilerSession::staticInit();
  ...
  MUcontext muContext;
  DRIVER_API_CALL(cuCtxCreate(&MUcontext, 0, cuDevice));
  ...
  if (!runTestWithUserRange(deviceNum, metricNames, MUcontext, false)) {
    LOG(ERROR) << "Failed to profiler test benchmark in user range";
  } else if (!runTestWithAutoRange(deviceNum, metricNames, MUcontext, false)) {
    LOG(ERROR) << "Failed to profiler test benchmark in auto range";
  } else if (!runTestWithUserRange(deviceNum, metricNames, MUcontext, true)) {
    LOG(ERROR) << "Failed to profiler test benchmark in user range async";
  } else if (!runTestWithAutoRange(deviceNum, metricNames, MUcontext, true)) {
    LOG(ERROR) << "Failed to profiler test benchmark in auto range async";
  }

  MuptiRBProfilerSession::deInitMupti();
#else
  LOG(WARNING) << "MuptiRBProfilerSession is not supported.";
#endif // HAS_MUPTI_RANGE_PROFILER
  DRIVER_API_CALL(cuCtxDestroy(MUcontext));
