...
#if defined(USE_MUPTI_RANGE_PROFILER) && defined(MUSART_VERSION) && MUSART_VERSION > 10000
#include <nvperf_musa_host.h>
// #include <nvperf_host.h>
#include <nvperf_target.h>
#endif // musa version > 10.00 and < 11.04
#endif // HAS_MUPTI
...
// going to be used by the MuptiRangeProfiler. These included calls
// to NVIDIA PerfWorks APIs.
namespace nvperf {


// Largely based on NVIDIA sample code provided with MUSA release
...
// the MUPTI profiler metric names this should be okay.

// Only supported on MUSA RT Version between 10.0 and 11.04.
// After MUSA RT 11.04, the structure has changed.
...
bool getRawMetricRequests(
  ...
  if (rawMetricRequests.size() == 0) {
    LOG(WARNING) << "MUPTI Profiler was unable to configure any metrics";
    return false;
  }
  return true;
}

// Setup MUPTI Profiler Config Image
bool getProfilerConfigImage(
  ...
  if (!NVPW_CALL(
        NVPW_MUSA_MetricsContext_Create(&metricsContextCreateParams))) {
    return false;
  }
  ...
  // Starting MUSA 11.4 the metric config create call and struct has changed
#if MUSART_VERSION < 11040
...
#else
   if (!NVPW_CALL(NVPW_MUSA_RawMetricsConfig_Create(&metricsConfigOptions))) {
     return false;
   }
