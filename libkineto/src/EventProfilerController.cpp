...
// We've observed deadlocks in Musa11 in libmusa / libmupti..
namespace detail {
...
std::map<MUcontext, unique_ptr<EventProfilerController>>& profilerMap() {
  static std::map<MUcontext, unique_ptr<EventProfilerController>> instance;
  return instance;
}
...
void EventProfilerController::start(MUcontext ctx, ConfigLoader& configLoader) {
...
// Must be called under lock
void EventProfilerController::stopIfEnabled(MUcontext ctx) {
...
void EventProfilerController::profilerLoop() {
  ...
  setThreadName("MUPTI Event Profiler");
