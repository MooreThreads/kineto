...
void Event::printSamples(ostream& s, MUdevice device) const {
...
Metric::Metric(
    ...
    : name(std::move(name)),
      ...
      evalMode_(eval_mode),
      muptiMetrics_(mupti_metrics),
      valueKind_(muptiMetrics_.valueKind(id)) {}

// Return per-SM vector as well as total
struct Metric::CalculatedValues Metric::calculate(
    map<MUpti_EventID, Event>& event_map,
  ...
  SampleValue total = muptiMetrics_.calculate(
      id_, valueKind_, events_, ev_values, sample_duration.count());
  if (evalMode_ & MUPTI_METRIC_EVALUATION_MODE_AGGREGATE) {
...
EventGroupSet::EventGroupSet(
    ...
    map<MUpti_EventID, Event>& events,
    MuptiEventApi& mupti)
    : set_(set), events_(events), muptiEvents_(mupti), enabled_(false) {
  for (int g = 0; g < set.numEventGroups; g++) {
    MUpti_EventGroup grp = set.eventGroups[g];
    // Profile all domain instances
    muptiEvents_.enablePerInstance(grp);
    uint32_t instance_count = muptiEvents_.instanceCount(grp);
    for (const auto& id : muptiEvents_.eventsInGroup(grp)) {
...
EventGroupSet::~EventGroupSet() {
  // Disable EventGroupSet in Mupti.
  if (enabled_) {
    setEnabled(false);
  }
}
...
void EventGroupSet::setEnabled(bool enabled) {
  if (enabled && !enabled_) {
    muptiEvents_.enableGroupSet(set_);
  } else if (!enabled && enabled_) {
    muptiEvents_.disableGroupSet(set_);
  }
  enabled_ = enabled;
}
...
void EventGroupSet::collectSample() {
  ...
  for (int g = 0; g < set_.numEventGroups; g++) {
    MUpti_EventGroup grp = set_.eventGroups[g];
    for (const auto& id : muptiEvents_.eventsInGroup(grp)) {
      Event& ev = events_[id];
      vector<int64_t> vals(ev.instanceCount);
      // FIXME: Use muptiEventGroupReadAllEvents
      muptiEvents_.readEvent(grp, id, vals);

      if (VLOG_IS_ON(0)) {
        for (int64_t v : vals) {
          if (v == MUPTI_EVENT_OVERFLOW) {
  ...
  if (VLOG_IS_ON(1)) {
    ...
    VLOG(1) << "Device " << muptiEvents_.device() << " Sample (us): "
...
void EventGroupSet::printDescription(ostream& s) const {
  for (int g = 0; g < set_.numEventGroups; g++) {
    s << "  Events in group " << g << ": ";
    for (const auto& id : muptiEvents_.eventsInGroup(set_.eventGroups[g])) {
...
void EventProfiler::initEvents(const std::set<std::string>& eventNames) {
  ...
  for (const auto& name : eventNames) {
    events_.emplace(muptiEvents_->eventId(name), name);
  }
}
...
void EventProfiler::initMetrics(const std::set<std::string>& metricNames) {
  ...
  for (const auto& metric_name : metricNames) {
    MUpti_MetricID metric_id = muptiMetrics_->idFromName(metric_name);
    if (metric_id == ~0) {
      continue;
    }

    const auto& events = muptiMetrics_->events(metric_id);
    vector<MUpti_EventID> event_ids;
    event_ids.reserve(events.size());
    for (const auto& pair : events) {
      MUpti_EventID id = pair.first;
    ...
    metrics_.emplace_back(
        ...
        metric_id,
        event_ids,
        muptiMetrics_->evaluationMode(metric_id),
        *muptiMetrics_);
  }
}
...
bool EventProfiler::initEventGroups() {
  sets_.clear();
  if (eventGroupSets_) {
    muptiEvents_->destroyGroupSets(eventGroupSets_);
  ...
  eventGroupSets_ = muptiEvents_->createGroupSets(ids);
  VLOG(0) << "Number of group sets: " << eventGroupSets_->numSets;
  for (int i = 0; i < eventGroupSets_->numSets; i++) {
    sets_.push_back(
        EventGroupSet(eventGroupSets_->sets[i], events_, *muptiEvents_));
  }
  return !sets_.empty();
}
...
EventProfiler::EventProfiler(
    ...
    : muptiEvents_(std::move(mupti_events)),
      muptiMetrics_(std::move(mupti_metrics)),
      loggers_(loggers),
...
void EventProfiler::printAllSamples(ostream& s, MUdevice device) const {
