...
constexpr char kMuptiProfilerPerKernelKey[] =
...
bool MuptiRangeProfilerConfig::handleOption(
...
void MuptiRangeProfilerConfig::printActivityProfilerConfig(
    std::ostream& s) const {
  if (activitiesMuptiMetrics_.size() > 0) {
    s << "Mupti Profiler metrics : "
      << fmt::format("{}", fmt::join(activitiesMuptiMetrics_, ", "))
      << std::endl;
    s << "Mupti Profiler measure per kernel : " << muptiProfilerPerKernel_
      << std::endl;
    s << "Mupti Profiler max ranges : " << muptiProfilerMaxRanges_ << std::endl;
  }
}

void MuptiRangeProfilerConfig::setActivityDependentConfig() {}

void MuptiRangeProfilerConfig::registerFactory() {
  Config::addConfigFactory(kMuptiProfilerConfigName, [](Config& cfg) {
    return new MuptiRangeProfilerConfig(cfg);
  });
}
