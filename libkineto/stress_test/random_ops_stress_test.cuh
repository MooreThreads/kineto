...
struct stress_test_args {
    ...
    // Can improve event density. Musa streams per worker
    uint32_t num_musa_streams {1};

    // Simulates musa mallocs happening in the PT Musa Cache Allocator
    double prob_musa_malloc {0.001};

    // The min number of compute iterations in the musa kernel. If high
    // this reduces event density.
    uint32_t min_iters_kernel {1};

    // The max number of compute iterations in the musa kernel. If high
    // this reduces event density.
    ...
    // with cache allocation (e.g. musaHostAlloc)
    ...
    // If true, we pre-allocate MUSA streams and reuse them throughout
    // the experiment
    ...
    // If true, we use musaGetMemInfo throughout the stress test to
    ...
    // Size of the MUPTI activity buffer in MB. If it's 0, we don't
    // explicitly set a value
    uint32_t mupti_buffer_mb {0};

    /* VARIABLES */

    // The MUSA streams vector
    musaStream_t *compute_streams {nullptr};

    // The explicit memcpy stream
    musaStream_t *memcpy_streams {nullptr};

    // The explicit UVM stream
    musaStream_t *uvm_streams {nullptr};
