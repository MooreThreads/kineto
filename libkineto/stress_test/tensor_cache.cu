...
void add_pairs_to_tensor_cache(tensor_cache_args cache_args, uint32_t
  ...
  for (uint32_t i = num_current_pairs;
    ...
    checkMusaStatus(
        musaMalloc(&p_memory_pool[i].d_A, num_elements * sizeof(float)), __LINE__);
    checkMusaStatus(
        musaMalloc(&p_memory_pool[i].d_B, num_elements * sizeof(float)), __LINE__);
    checkMusaStatus(
        musaMalloc(&p_memory_pool[i].d_C, num_elements * sizeof(float)), __LINE__);
    ...
    if (((float)(rand() % 32767) / 32767.0) < cache_args.prob_h2d) {
      ...
      checkMusaStatus(musaHostAlloc(&p_memory_pool[i].h_A, num_elements * sizeof(float), musaHostAllocDefault), __LINE__);
      checkMusaStatus(musaHostAlloc(&p_memory_pool[i].h_B, num_elements * sizeof(float), musaHostAllocDefault), __LINE__);
...
void free_and_realloc_tensor_pairs(tensor_pair *tensor_pair, musaStream_t stream) {
// Older MUSA versions don't know about async malloc and free
#if defined(MUSA_VERSION) && MUSA_VERSION > 11000 && defined(ASYNC_MALLOC)

  checkMusaStatus(
    musaFreeAsync(tensor_pair->d_A, stream),
        __LINE__);
  checkMusaStatus(
    musaFreeAsync(tensor_pair->d_B, stream),
        __LINE__);
  checkMusaStatus(
    musaFreeAsync(tensor_pair->d_C, stream),
        __LINE__);
  ...
  checkMusaStatus(musaFree(tensor_pair->d_A), __LINE__);
  checkMusaStatus(musaFree(tensor_pair->d_B), __LINE__);
  checkMusaStatus(musaFree(tensor_pair->d_C), __LINE__);

  // Allocate device buffers
  uint32_t num_elements = tensor_pair->n_elements;
  checkMusaStatus(musaMalloc(&tensor_pair->d_A,
    num_elements * sizeof(float)),
    __LINE__);
  checkMusaStatus(musaMalloc(&tensor_pair->d_B,
    num_elements * sizeof(float)),
    __LINE__);
  checkMusaStatus(musaMalloc(&tensor_pair->d_C,
    num_elements * sizeof(float)),
    __LINE__);

#endif // MUSA_VERSION >= 11000

  if (tensor_pair->b_copy_h2d) {
    checkMusaStatus(musaFreeHost(tensor_pair->h_A), __LINE__);
    checkMusaStatus(musaFreeHost(tensor_pair->h_B), __LINE__);

    checkMusaStatus(musaHostAlloc(&tensor_pair->h_A, num_elements * sizeof(float), musaHostAllocDefault), __LINE__);
    checkMusaStatus(musaHostAlloc(&tensor_pair->h_B, num_elements * sizeof(float), musaHostAllocDefault), __LINE__);
...
void free_tensor_cache() {
  for (uint32_t i = 0; i < num_tensor_pairs; ++i) {
    checkMusaStatus(musaFree(p_memory_pool[i].d_A), __LINE__);
    checkMusaStatus(musaFree(p_memory_pool[i].d_B), __LINE__);
    checkMusaStatus(musaFree(p_memory_pool[i].d_C), __LINE__);

    if (p_memory_pool[i].b_copy_h2d) {
      if (p_memory_pool[i].h_A) {
        checkMusaStatus(musaFreeHost(p_memory_pool[i].h_A), __LINE__);
      }

      if (p_memory_pool[i].h_B) {
        checkMusaStatus(musaFreeHost(p_memory_pool[i].h_B), __LINE__);
      }
    }
  }
