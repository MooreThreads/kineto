...
void run_stress_test(
  ...
  checkMusaStatus(musaMemGetInfo(&mem_free, &mem_total), __LINE__);
  mem_used_before = (mem_total - mem_free) / 1024 / 1024;

  // Create multiple streams
  musaStream_t* v_streams = NULL;
  musaStream_t memcpy_stream = NULL;
  musaStream_t uvm_stream = NULL;
  ...
  if (test_args.pre_alloc_streams) {
    v_streams = test_args.compute_streams + (thread_id * test_args.num_musa_streams);
    ...
    v_streams = (musaStream_t*)malloc(test_args.num_musa_streams * sizeof(musaStream_t));
    for (uint32_t i = 0; i < test_args.num_musa_streams; ++i) {
      checkMusaStatus(musaStreamCreate(v_streams + i), __LINE__);
    }

    if (test_args.use_memcpy_stream) {
      checkMusaStatus(musaStreamCreate(&memcpy_stream), __LINE__);
    }

    if (test_args.use_uvm_stream) {
      checkMusaStatus(musaStreamCreate(&uvm_stream), __LINE__);
    }
  }
  ...
  for (uint32_t i = 0; i < test_args.num_operations; ++i) {
    ...
    uint32_t stream_idx = local_pair_idx % test_args.num_musa_streams;
    musaStream_t current_stream = v_streams[stream_idx];
    musaStream_t current_memcpy_stream = v_streams[stream_idx];
    if (test_args.use_memcpy_stream) {
      current_memcpy_stream = memcpy_stream;
    }
    musaStream_t current_uvm_stream = v_streams[stream_idx];
    if (test_args.use_uvm_stream) {
      current_uvm_stream = uvm_stream;
    }

    // Check if we do a MUSA malloc
    if (((float)(rand_r(&rng_state) % 32767) / 32767.0) < test_args.prob_musa_malloc) {
      checkMusaStatus(musaDeviceSynchronize(), __LINE__);
    ...
    if ((kernel_args.uvm_len > 0) && (b_uvm_kernel)) {
      if (b_do_memset) {
        ...
        memset((void*)test_args.uvm_a, 42, kernel_args.len * sizeof(float));
        // checkMusaStatus(musaMemset((void*)test_args.uvm_a, 42, kernel_args.len * sizeof(float)));
        // checkMusaStatus(musaMemset((void*)test_args.uvm_a, 42, kernel_args.len * sizeof(float)));
      } else {
      ...
      if (b_do_memset) {
        checkMusaStatus(musaMemset((void*)kernel_args.d_a, 42, kernel_args.len * sizeof(float)));
        checkMusaStatus(musaMemset((void*)kernel_args.d_b, 42, kernel_args.len * sizeof(float)));
        checkMusaStatus(musaMemset((void*)kernel_args.d_c, 42, kernel_args.len * sizeof(float)));
      } else {
    ...
    if ((i % test_args.num_iters_nccl_sync == 0) && (test_args.is_multi_rank)) {
      ...
      checkMusaStatus(musaMemcpy(pBuffNCCLSend, p_memory_pool[pair_idx].d_C,
          szTransfer, musaMemcpyDeviceToDevice), __LINE__);
      ...
      checkMusaStatus(musaStreamSynchronize(current_stream), __LINE__);
      checkMusaStatus(musaMemcpy(p_memory_pool[pair_idx].d_C, pBuffNCCLRecv,
          szTransfer, musaMemcpyDeviceToDevice), __LINE__);
    }
  ...
  for (int i = 0; i < test_args.num_musa_streams; ++i) {
    checkMusaStatus(musaStreamSynchronize(v_streams[i]), __LINE__);
  }

  if (test_args.use_memcpy_stream) {
    checkMusaStatus(musaStreamSynchronize(memcpy_stream), __LINE__);
  }

  if (test_args.use_uvm_stream) {
    checkMusaStatus(musaStreamSynchronize(uvm_stream), __LINE__);
  }
  ...
  if (!test_args.pre_alloc_streams) {
    for (int i = 0; i < test_args.num_musa_streams; ++i) {
      checkMusaStatus(musaStreamDestroy(v_streams[i]), __LINE__);
    }

    if (test_args.use_memcpy_stream) {
      checkMusaStatus(musaStreamDestroy(memcpy_stream), __LINE__);
    }

    if (test_args.use_uvm_stream) {
      checkMusaStatus(musaStreamDestroy(uvm_stream), __LINE__);
    }
...
// In case MUPTI compresses data using kernel name as a key to a hash map
