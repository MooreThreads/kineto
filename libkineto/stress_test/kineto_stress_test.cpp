...
void read_inputs_from_json(std::string sJsonFile, stress_test_args *test_args,
  ...
  if (!fJson.fail()) {
    ...
    test_args->num_musa_streams = (uint32_t)jsonTestArgs["num_musa_streams"].asInt();
    test_args->prob_musa_malloc = (double)jsonTestArgs["prob_musa_malloc"].asDouble();
    ...
    test_args->mupti_buffer_mb = (uint32_t)jsonTestArgs["mupti_buffer_mb"].asInt();
...
void trace_collection_thread(uint32_t trace_length_us,
  uint32_t mupti_buffer_mb) {

  if (mupti_buffer_mb > 0) {
    // Configure MUPTI buffer sizes
    size_t attrValue = 0, attrValueSize = sizeof(size_t);
    attrValue = (size_t)(mupti_buffer_mb * 1024 * 1024);
    muptiActivitySetAttribute(MUPTI_ACTIVITY_ATTR_DEVICE_BUFFER_SIZE,
...
void uvm_allocation_thread(stress_test_args *test_args) {
    ...
    int currentDevice = 0;
    checkMusaStatus(musaGetDevice(&currentDevice), __LINE__);
    checkMusaStatus(musaMallocManaged((void**)&test_args->uvm_a, alloc_size, musaMemAttachGlobal), __LINE__);
    checkMusaStatus(musaMallocManaged((void**)&test_args->uvm_b, alloc_size, musaMemAttachGlobal), __LINE__);
    checkMusaStatus(musaMemAdvise((void*)test_args->uvm_a, alloc_size, musaMemAdviseSetPreferredLocation, musaCpuDeviceId), __LINE__);
    checkMusaStatus(musaMemAdvise((void*)test_args->uvm_b, alloc_size, musaMemAdviseSetPreferredLocation, musaCpuDeviceId), __LINE__);
    checkMusaStatus(musaMemAdvise((void*)test_args->uvm_a, alloc_size, musaMemAdviseSetAccessedBy, currentDevice), __LINE__);
    checkMusaStatus(musaMemAdvise((void*)test_args->uvm_b, alloc_size, musaMemAdviseSetAccessedBy, currentDevice), __LINE__);
...
void create_musa_streams(stress_test_args& test_args) {
  test_args.compute_streams = (musaStream_t*)malloc(test_args.num_musa_streams *
    test_args.num_workers * sizeof(musaStream_t));
  for (uint32_t i = 0; i < test_args.num_musa_streams * test_args.num_workers; ++i) {
    checkMusaStatus(musaStreamCreateWithFlags(test_args.compute_streams + i, musaStreamNonBlocking), __LINE__);
  }

  if (test_args.use_memcpy_stream) {
    test_args.memcpy_streams = (musaStream_t*)malloc(test_args.num_workers * sizeof(musaStream_t));
    for (uint32_t i = 0; i < test_args.num_workers; ++i) {
      checkMusaStatus(musaStreamCreateWithFlags(test_args.memcpy_streams + i, musaStreamNonBlocking), __LINE__);
    }
  }

  if (test_args.use_uvm_stream) {
    test_args.uvm_streams = (musaStream_t*)malloc(test_args.num_workers * sizeof(musaStream_t));
    for (uint32_t i = 0; i < test_args.num_workers; ++i) {
      checkMusaStatus(musaStreamCreateWithFlags(test_args.uvm_streams + i, musaStreamNonBlocking), __LINE__);
    }
  }
}

void cleanup_musa_streams(stress_test_args& test_args) {
  for (int i = 0; i < test_args.num_musa_streams * test_args.num_workers; ++i) {
    checkMusaStatus(musaStreamSynchronize(test_args.compute_streams[i]), __LINE__);
    checkMusaStatus(musaStreamDestroy(test_args.compute_streams[i]), __LINE__);
  }
  ...
  if (test_args.use_memcpy_stream) {
    for (int i = 0; i < test_args.num_workers; ++i) {
      checkMusaStatus(musaStreamSynchronize(test_args.memcpy_streams[i]), __LINE__);
      checkMusaStatus(musaStreamDestroy(test_args.memcpy_streams[i]), __LINE__);
    }
  ...
  if (test_args.use_uvm_stream) {
    for (int i = 0; i < test_args.num_workers; ++i) {
      checkMusaStatus(musaStreamSynchronize(test_args.uvm_streams[i]), __LINE__);
      checkMusaStatus(musaStreamDestroy(test_args.uvm_streams[i]), __LINE__);
    }
...
int main(int argc, char *argv[]) {
  ...
  if (test_args.is_multi_rank) {
    checkMusaStatus(musaSetDevice(test_args.rank));
    ...
    checkMusaStatus(musaMalloc(&pBuffNCCLSend, test_args.sz_nccl_buff_KB * 1024), __LINE__);
    checkMusaStatus(musaMalloc(&pBuffNCCLRecv, test_args.sz_nccl_buff_KB * 1024), __LINE__);
    checkMusaStatus(musaMemset(pBuffNCCLSend, 0, test_args.sz_nccl_buff_KB * 1024), __LINE__);
    checkMusaStatus(musaMemset(pBuffNCCLRecv, 0, test_args.sz_nccl_buff_KB * 1024), __LINE__);

    // Make sure all the processes have allocated this memory
    MPICHECK(MPI_Barrier(MPI_COMM_WORLD));
  }

  // Pre-allocate MUSA streams
  if (test_args.pre_alloc_streams) {
    create_musa_streams(test_args);
  }
  ...
  // Destroy MUSA streams
  if (test_args.pre_alloc_streams) {
    cleanup_musa_streams(test_args);
  }
  ...
  // Free UVM
  if (test_args.use_uvm_buffers) {
    checkMusaStatus(musaFree(test_args.uvm_a), __LINE__);
    checkMusaStatus(musaFree(test_args.uvm_b), __LINE__);
  }

  if (test_args.is_multi_rank) {
    checkMusaStatus(musaFree(pBuffNCCLRecv));
    checkMusaStatus(musaFree(pBuffNCCLSend));
