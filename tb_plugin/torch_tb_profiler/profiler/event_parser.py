...
from .trace import BaseEvent, DurationEvent, EventTypes, KernelEvent, McclOpNameSet, GlooOpNameSet

logger = utils.get_logger()

CommLibTypes = IntEnum('CommLibTypes', ['Mccl', 'Gloo'], start=0)
...
class NodeParserMixin:
    ...
    def parse_nodes(self, events: Iterable[BaseEvent]):
        ...
        if CommLibTypes.Mccl in self.comm_lib:
            for event in events:
    ...
    def _parse_node(self,
        ...
        if event.type in [EventTypes.KERNEL, EventTypes.MEMCPY, EventTypes.MEMSET]:
            ...
            if event.name in McclOpNameSet or event.name in GlooOpNameSet:
                comm_node = CommunicationNode.create(event)
                if event.name in McclOpNameSet:
                    self.comm_lib.add(CommLibTypes.Mccl)
...
class StepParser:
    ...
    def _parse_step(self, event: DurationEvent, comm_nodes: Dict[int, CommunicationNode]):
        ...
        if evt_type == EventTypes.KERNEL:
            ...
            if event.name in GlooOpNameSet or event.name in McclOpNameSet:
                self.role_ranges[ProfileRole.Communication].append((ts, ts + dur))
            else:
